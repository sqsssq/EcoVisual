    ...
    d3.json(fileURL).then((coor) => {
        d3.csv('data/box_calc.csv').then((rectdata) => {
            ...
            for (let i in edges) {
                ...
                if (edges[i].source < sel) {
                    temp.source = web[edges[i].source];
                }

                if (edges[i].target < sel) {
                    temp.target = web[edges[i].target];
                }
                if (edges[i].source > sel) {
                    temp.source = web[edges[i].source - 1];
                }

                if (edges[i].target > sel) {
                    temp.target = web[edges[i].target - 1];
                }
            ...
            console.log(edx);
            //(1)创建箭头
            ssvg.append("svg:defs").selectAll("marker")
            ...
            //(2)根据连线类型引用上面创建的标记
            var path = ssvg.append("svg:g").selectAll("path")
                .data(edx)
                ...
                .attr('stroke-width', d => valScale(d.value));
