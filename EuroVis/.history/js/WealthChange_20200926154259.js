...
function PaintBackground(opac, px, pn) {
    ...
    d3.csv('data/box_calcr.csv', function (data) {
        ...
        var xAxis = d3.svg.axisBottom().scale(p_xscale).ticks(0).tickFormat(d3.format("d"));
        var yAxis = d3.svg.axisLeft().scale(p_yscale).ticks(10).tickFormat(d3.format()).orient("left"); //添加一个g用于放x轴
