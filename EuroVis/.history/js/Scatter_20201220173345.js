        ...
        d3.csv('data/box_calc.csv').then((rectdata) => {
            ...
            for (let i in dicpos) {
                ...
                if (dicpos[i].wealthOut_avg > 0) {
                    temp2.target.x += Math.cos(outScaleZheng(dicpos[i].wealthOut_avg)) * diccircle[i].dcnt;
                    temp2.target.y += Math.sin(outScaleZheng(dicpos[i].wealthOut_avg)) * diccircle[i].dcnt;
                }
                if (dicpos[i].wealthOut_avg <= 0) {
                    temp2.target.x += Math.cos(outScaleFu(dicpos[i].wealthOut_avg)) * diccircle[i].dcnt;
                    temp2.target.y += Math.sin(outScaleFu(dicpos[i].wealthOut_avg)) * diccircle[i].dcnt;
                }
                dicline.push(temp1);
                dicline.push(temp2);
