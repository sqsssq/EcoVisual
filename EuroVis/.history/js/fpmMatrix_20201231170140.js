            ...
            d3.csv("data/fpm01.csv").then((data3) => {
                ...
                for (let i in typeSimilar2) {
                    ...
                    sim.push({
                        ...
                        cnt: typeSimilar2[i].cnt / (typeCount[i] + typeCount[j] - typeSimilar2[i].cnt),
                        startAngle: Math.PI / 2,
                        endAngle: Math.PI * 3 / 2
                    });
                    simMax = Math.max(simMax, typeSimilar2[i].cnt / (typeCount[i] + typeCount[j] - typeSimilar2[i].cnt));
                    simMin = Math.min(simMin, typeSimilar2[i].cnt / (typeCount[i] + typeCount[j] - typeSimilar2[i].cnt));
                }
                let simScale = d3.scaleLinear()
                .domain([simMin, simMax])
                .range([1, 10])
