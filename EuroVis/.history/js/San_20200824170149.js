...
var title = ['wealth', 'work', 'health', 'insurance', 'loan', 'investment', 'risk', 'disaster', 'lottery', 'ill', 'unemploy', 'rank', '风险偏好']
    ...
    d3.csv("data/box.csv", function (d1) {
        ...
        d3.json(coorp, function (coork) {
            d3.csv("data/box_calc.csv", function (RectInData) {
                d3.json('data/ts/alldriving.json', function (alldata) {                // console.log(RectInData)
                var coor = coork;

                if (Rect_g != 0) Rect_g.remove()
                if (text_g != 0) text_g.remove()

                Rect_g = svg.append('g').attr("transform", "translate(" + 0 + "," + 10 + ")")
