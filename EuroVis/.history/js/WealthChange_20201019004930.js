    ...
    d3.json(data_address).then((type_data) => {
        d3.csv('data/box_calc.csv').then((data) => {
            d3.csv('data/treatment.csv').then(treatData => {
                ...
                var diaganlData = new Object();
                for (let i in nameLineData) {
                    for (let j = 1; j < 20; ++j) {
                        // console.log(j * 1000000 + nameLineData[j] * 10000 + (j + 1) * 100 + nameLineData[j + 1])
                        if (typeof (diaganlData[j * 1000000 + nameLineData[i][j] * 10000 + (j + 1) * 100 + nameLineData[i][j + 1]]) == 'undefined')
                            diaganlData[j * 1000000 + nameLineData[i][j] * 10000 + (j + 1) * 100 + nameLineData[i][j + 1]] = 0;
                        diaganlData[j * 1000000 + nameLineData[i][j] * 10000 + (j + 1) * 100 + nameLineData[i][j + 1]]++;
                    }
                }
                var diagnalDrawData = new Object();
                for (let i in diaganlData) {
                    diagnalDrawData[i] = {
                        ...
                        source: {
                            x: p_xscale(parseInt(i / 1000000)),
                            y: p_yscale(parseInt((i % 1000000) / 10000))
                        },
                        target: {
                            x: p_xscale(parseInt((i % 10000) / 100)),
                            y: p_yscale(parseInt(i % 100))
                        }
                    };
                }
                console.log(diagnalDrawData)
