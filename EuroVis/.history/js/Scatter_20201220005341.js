...
function DrawGlyph() {
    d3.json(fileURL).then((gdata) => {
        d3.csv('data/box_calc.csv').then((rectdata) => {
            ...
            for (let i in dicpos) {
                var temp1 = {
                    source: {x: diccircle[i].pos[0], y: diccircle[i].pos[1]},
                    target: {x: diccircle[i].pos[0] + Math.cos(inScale(dicpos[i].wealthIn_avg)) * diccircle[i].dcnt, y: diccircle[i].pos[1] + Math.sin(inScale(dicpos[i].wealthIn_avg)) * diccircle[i].dcnt}
                }
                // var temp2 = {
                //     source: {x: dicpos[i].pos[0], y: dicpos[i].pos[1]},
                //     target: 0
                // }
                dicline.push(temp1);
