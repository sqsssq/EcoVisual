...
function DrawGlyph() {
    d3.json(fileURL).then((gdata) => {
        d3.csv('data/box_calc.csv').then((rectdata) => {
            ...
            for (let i in dicpos) {
                var temp1 = {
                    start: inScale(dicpos[i].wealthIn_box[0]),
                    end: inScale(dicpos[i].wealthIn_box[1]),
                    r: diccircle[i].dcnt - ((boxstep - boxandline) / 2 + boxandline) * linescale(dicpos[i].cnt)
                }
                var temp2 = {
                    start: inScale(dicpos[i].wealthIn_box[3]),
                    end: inScale(dicpos[i].wealthIn_box[4]),
                    r: diccircle[i].dcnt - ((boxstep - boxandline) / 2 + boxandline) * linescale(dicpos[i].cnt)
                }
                var temp3 = {
                    start: inScale(dicpos[i].wealthIn_box[1]),
                    end: inScale(dicpos[i].wealthIn_box[3]),
