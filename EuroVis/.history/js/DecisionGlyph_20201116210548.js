        ...
        d3.csv('data/box_calc.csv').then((rectdata) => {
            ...
            for (let i in boxInnerData) {
                ...
                for (let j in boxInnerData[i]) {
                    ...
                    var rd = Math.atan(llong / lx);
                    console.log(Math.sin(i * onePiece))

                    var inLine = {
                        x1: vx - (llong) * llong / Math.sqrt(llong * llong + lx * lx),
                        y1: vy - (llong) * lx / Math.sqrt(llong * llong + lx * lx),
                        x2: vx + (llong) * llong / Math.sqrt(llong * llong + lx * lx),
                        y2: vy + (llong) * lx / Math.sqrt(llong * llong + lx * lx)
                    };
                ...
                //     y2: lk[4] * Math.cos(i * onePiece),
                // });
                var rd1 = Math.atan(llong / lk[1]);
                var rd2 = Math.atan(llong / lk[3]);
                ...
                boxLineData.push({
                    x1: vx1 - (llong) * llong / Math.sqrt(llong * llong + lk[1] * lk[1]),
                    y1: vy1 - (llong) * lk[1] / Math.sqrt(llong * llong + lk[1] * lk[1]),
                    x2: vx2 - (llong) * llong / Math.sqrt(llong * llong + lk[3] * lk[3]),
                    y2: vy2 - (llong) * lk[3] / Math.sqrt(llong * llong + lk[3] * lk[3]),
                })
                boxLineData.push({
                    x1: vx1 + (llong) * llong / Math.sqrt(llong * llong + lk[1] * lk[1]),
                    y1: vy1 + (llong) * lk[1] / Math.sqrt(llong * llong + lk[1] * lk[1]),
