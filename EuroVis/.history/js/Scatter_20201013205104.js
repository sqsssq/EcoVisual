...
function Rader(data, x, y, zoom) {
    ...
    for (var k = level; k > 0; k--) {
        var webs = '',
            webPoints = [];
        var r = (radius * 7 / 6) / level * k + radius / 3;
    ...
    for (var i = 0; i < values.length; i++) {
        ...
        for (var k = 0; k < total; k++) {
            ...
            var r = radius * ((value[k]) / rangeMax) + radius / 3;
    ...
    for (var i = 0; i < total; i++) {
        ...
        // if (i == 1 || i == 2) {
        for (k = level; k > 0; k--) {
            var r = radius / level * (k - 0.5) + radius / 3;
            var x = r * Math.sin(i * onePiece),
                y = r * Math.cos(i * onePiece);
    ...
    liner.append('circle')
        .attr('cx', 0)
        .attr('cy', 0)
        .attr('r', outCircleRadius + radius / 3)
        .attr('stroke-width', 1)
        .attr('fill', 'none')
        .attr('stroke', 'gray');
    return main;
