...
function DrawSankey(data2, data3) {
    d3.csv("data/box_calc_rank.csv").then((data) => {
        if (Sankey_g != 0) {
            Sankey_g.remove();
            Sankey_g = 0;
        ...
        patternRect = Sankey_g.selectAll("#patternRect")
            .attr('id', 'patternRect')
            .data(patternSelect)
            .enter()
            .append('rect')
        ...
        Sankey_g.selectAll("#patternText")
            .attr('id', 'patternText')
            .data(patternSelect)
            .enter().append('text')
        ...
        //     Sankey_g.selectAll('#rectL').attr('id', 'rectL')
        //         .data(d3.range(100))
        //         .enter()
        //         .append('rect')
        //         .attr('x', (d, i) => {
        //             return i * stepL * 0.6 / 100 + 0.01 * widthMatrix + stepL * k;
        //         })
        //         .attr('y', 4)
        //         .attr('width', stepL * 0.8 / 100)
        //         .attr('height', 12)
        //         .attr("stroke", (d, i) => typeColor[k])
        //         .style('fill', (d, i) => typeColor[k])
        //         .attr('fill-opacity', 0.8)
        //         .attr('stroke-opacity', 0.8)
        //         .on('mouseover', d => {
        //             patternCir.attr('r', d => {
        ...
        //                 }
        //             })
        //             decisionDiagonal.attr('stroke-opacity', d => {
        //                 if (d.pattern == k) return 1;
        //                 else return 0;
        //             })
        //         })
        //         .on('mouseout', d => {
        ...
        //         .attr('x', 0.01 * widthMatrix + stepL * (k) + stepL * 0.6)
        //         .attr('y', 14)
        //         .attr('dx', 3)
        //         .attr('font-size', 10)
        //         .text("P" + k)
        ...
        // decisionDiagonal = Sankey_g.append("g")
        ...
        // patternCir = Sankey_g.selectAll('#patternCircle')
...
function DrawLine(data2, data3, patternSelect) {
    d3.csv("data/box_calc_rank.csv").then((data) => {
        if (Change_g != 0) {
            Change_g.remove();
            Change_g = 0;
        }
        Change_g = Sankey_g.append("g");
        // 连接线数据
        let connectData = new Object();
        // 节点数据
        let pointData = new Object();
