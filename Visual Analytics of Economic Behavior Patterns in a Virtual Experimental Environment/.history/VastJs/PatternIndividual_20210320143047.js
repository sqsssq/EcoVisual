    ...
    d3.csv('data/newdata_for_line.csv').then(wealth_data => {
        ...
        for (let i in personal_data) {
            ...
            const g_p = pie_g.selectAll("#flower_x" + (select_people_num).toString())
                ...
                .enter()
                .append('g').attr("transform", "translate(" + this.x_scale_wealth(parseInt(i) + 1) + "," + (this.y_scale_wealth(wealthList[i].y)) + ") rotate(-97.5, " + this.x_scale_wealth(parseInt(i) + 1) + "," + (this.y_scale_wealth(wealthList[i].y)) + ")")
            g_p.append('path')
...
function drawPersonalHorizon(code) {
    d3.csv('data/newdata_for_line.csv').then(wealth_data => {
        ...
        for (let i in horizon_data) {
            // if (parseInt(i) == 9) break;
            // console.log(i)
            drawHorizonR(horizon_data[i], max_gain, 20, 11 * parseInt(i) + 11, pie_g, select_people_num - 1);
        }
        pie_g.append('rect').attr('x', 20).attr('y', 11).attr('height', 132).attr('width', 235).attr('fill', 'none').attr('stroke', 'black').attr('stroke-width', 1);
    });
}
