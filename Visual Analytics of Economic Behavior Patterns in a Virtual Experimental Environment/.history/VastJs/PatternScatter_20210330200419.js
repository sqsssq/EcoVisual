...
function DrawAxis(scatter_data, flag) {
    d3.csv('data/newdata_for_line.csv').then((box_data) => {
        ...
        .domain([0, 1])
        .range([0, 30]);
        var x_tmp2 = d3.scaleLinear()
        .domain([0, 1])
        .range([xAxisWidth - 10 - 40, xAxisWidth - 10]);
        axis_g.append("g")
        .attr("transform", `translate(0,${y_scale_array[y_axis_type](0)})`)
        .call(d3.axisBottom(x_tmp1).ticks(0));
        axis_g.append("g")
        .attr("transform", `translate(${x_scale_array[x_scale_array.length - 1].range[0]},${y_scale_array[y_axis_type](0)})`)
        .call(d3.axisBottom(x_tmp2).ticks(0));
        // })
    })
}
