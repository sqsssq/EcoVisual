    ...
    d3.json('data/20210318_1.json').then((scatter_data) => {
        d3.csv('data/newdata_for_line.csv').then((rect_data) => {
            ...
            var yScale = d3.scaleLinear()
                .domain([min_y, max_y])
                .range([10, yAxisWidth - 10]);
            var start_max_Scale = d3.scaleQuantize()
                .domain([0, max_start])
                .range([1, 10]);
            var computeMax = d3.interpolate('gray', 'green');
            ...
            let patternScatter = scatter_g.selectAll("#scatterPattern")
                ...
                .attr('fill', (d, i) => {
                    ...
                    if (flag == 1) {
                        // if (d.start > 0)
                        //     return computeMax(Math.log10(start_max_Scale(d.start)));
                        // console.log()
                        if (d.start > 0)
                        console.log(Math.log10(start_max_Scale(d.start)), d.start, start_max_Scale(d.start))
