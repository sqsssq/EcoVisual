...
function drawPattern(move_x, move_y, selectData) {
    d3.csv('data/data_for_line.csv').then(data => {
        d3.csv('data/treatment.csv').then(treat => {
            ...
            p_g.append('text').text("P" + (countType - 1).toString()).attr('dx', '0.5em').attr('dy', '1.2em').attr('font-family', 'Georgia').attr('font-size', 20);
            let c_width = (widthGantt - 5 * 10) / 4;
            let c_height = heightGantt - 10 * 2;
            p_g.append('rect')
                .attr('x', 0)
            ...
            $(document).ready(function () {
                $("#ganttView").append('<div style="position: absolute; top: 355px; left: ' + (30 + move_x) + 'px; font-family: Georgia;">Process: <select name="select1" id="se" onclick="console.log(this.value)"><option value="1">start</option><option value="2">Work</option><option value="3">health</option><option value="6" selected>Investment</option><option value="1">start</option></select></div>');
                $("#ganttView").append('<div style="position: absolute; top: 355px; left: ' + (210 + move_x) + 'px; font-family: Georgia;">Round: <select name="select1" id="se" onclick="console.log(this.value)"><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5" selected>5</option></select></div>');
            
            })
            DrawCurveLine(selectData, move_x, move_y + 340, 4, 5);
