...
function drawFlower(code) {
    d3.csv('data/data_for_line.csv').then(wealth_data => {
        ...
        for (let i in horizon_data) {
            ...
            console.log(i)
            drawHorizonR(horizon_data[i], max_gain, 0, 9 * parseInt(i) + 3, pie_g);
        }
    });
}
...
function drawHorizonR(roundCount, max_num, move_x, move_y, compare_g) {
    ...
    horizon_g.append('rect').attr('x', x_scale.range()[0]).attr('y', 0).attr('height', horizon_height).attr('width', x_scale.range()[1] - x_scale.range()[0]).attr('fill', 'none').attr('stroke', 'black').attr('stroke-width', 2);
