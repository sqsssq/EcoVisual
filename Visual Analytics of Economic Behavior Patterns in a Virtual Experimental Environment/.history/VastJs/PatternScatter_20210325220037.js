...
function DrawScatterLine(code) {
    d3.json(fileURL).then((scatter_data) => {
        ...
        let mark_data = new Array();
        for (let i = 0; i < scatterLine_data.length - 1; ++i)
        {
            let tmp = [scatterLine_data[i], scatterLine_data[i + 1]];
            mark_data.push(tmp);
        }
        ...
        zoom_g.append('svg:defs').append('svg:marker')
            .attr('id', 'end-arrow')
    ...
    d3.json(fileURL).then((scatter_data) => {
        ...
        function brushed() {
            ...
            let type_g_k = zoom_g.append("g")
                .attr("id", "zoom_type" + (countType - 2).toString());
