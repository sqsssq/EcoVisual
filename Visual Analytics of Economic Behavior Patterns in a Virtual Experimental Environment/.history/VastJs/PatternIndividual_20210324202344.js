...
function globalWealth() {
    ...
    d3.select("#wealth_x_axis")
        .transition()
        .attr("transform", `translate(0, ${y_scale_Wealth(0)})`)
    ...
    d3.select("#box_line_h")
        .transition()
        .attr("y1", d => y_scale_Wealth(d.y))
        .attr("y2", d => y_scale_wealth(d.y));
    d3.select("#box_line_s")
        .transition()
    ...
    for (let i = 0; i < max_wealth_array.length; ++i) {
        ...
        for (let j = 1; j <= 20; ++j) {
            ...
            d3.select("#flower_sum" + i.toString() + "_" + j.toString())
                .transition()
                .attr("transform", `translate(${this.x_scale_wealth(j)}, ${y_scale_Wealth(parseFloat(v))})`);
        }
    }
}
