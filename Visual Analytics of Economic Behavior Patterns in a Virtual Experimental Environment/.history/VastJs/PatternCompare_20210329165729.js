...
var lineLegendType_2 = ['Work', 'Health Invest', 'Repayment', 'Loan', 'Investment', 'Venture', 'Insurance', 'Lottery', 'Disease', 'Illness', 'Risk', 'Patience'];
var lineNameLegend = [
...
function drawPattern(move_x, move_y, selectData, flag) {
    d3.csv('data/newdata_for_line.csv').then(data => {
        d3.csv('data/treatment.csv').then(treat => {
            ...
            p_g.append('text').text("Pattern " + (countType - 1).toString()).attr('dx', '0.5em').attr('dy', '1.2em').attr('font-family', 'Georgia').attr('font-size', 22).attr('font-weight', 'bold');
            ...
            let changHight = c_height - this.outerRadius - 60;
            ...
            drawArea(compare_g, timeLine, profitLine, max_people, max_profit, min_profit, c_height / 6 + 50, move_x, countType - 1);
        })
    })
}
