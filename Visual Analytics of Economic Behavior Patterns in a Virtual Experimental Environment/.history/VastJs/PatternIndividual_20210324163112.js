...
function drawFlower(code, select_round) {
    d3.csv('data/newdata_for_line.csv').then(wealth_data => {
        ...
        for (let p in personal_data) {
            ...
            max_receive = Math.max(max_receive, dpend);
            min_receive = Math.min(min_receive, dpend);
            wealthList.push({
                y: dpend,
                x: parseInt(p) + 1
            });
        ...
        for (let i in personal_data) {
            ...
            const g_p = pie_g.selectAll(".flower_x" + (parseInt(select_people_num) - 1).toString())
                .attr('class', 'flower_x' + (parseInt(select_people_num) - 1).toString())
                .append('g')
                .attr("id", "flower_sum" + (parseInt(select_people_num) - 1).toString()).attr("transform", "translate(" + this.x_scale_wealth(parseInt(i) + 1) + "," + (this.y_scale_wealth(wealthList[i].y)) + ") rotate(-105, " + 0 + "," + 0 + ")")
...
function globalWealth() {
    ...
    for (let i = 0; i < max_wealth_array.length; ++i) {
        ...
        d3.select("#flower_sum" + i.toString())
        .transition()
        .attr("transform", `translate(${this.x_wealth_scale()}, )`)
    }
}
