    ...
    d3.csv('data/newdata_for_line.csv').then(data => {
        d3.csv('data/treatment.csv').then(treat => {
            ...
            for (let i in line_type_width) {
                ...
                lineType_code.push(i[0] + i[2]);
                if (typeof(type_code[i[0] + i[2]]) == 'undefined') {
                    type_code[i[0] + i[2]] = 0;
                }
                if (line_type_width[i].w > type_code[i[0] + i[2]]) {
                    type_code[i[0] + i[2]] = line_type_width[i].w;
                }
            }
            console.log(lineType_code)
