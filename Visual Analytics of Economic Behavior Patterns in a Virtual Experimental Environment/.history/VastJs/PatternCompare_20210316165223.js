        ...
        d3.csv('data/treatment.csv').then(treat => {
            ...
            for (let i = 0; i < lineLegend.length; ++i) {
                ...
                for (let j in selectData) {
                    ...
                    peopleHistory[selectData[j].id][selectData[j].lun][i].d1 = i;
                    peopleHistory[selectData[j].id][selectData[j].lun][i].d2 = peopleHistory[selectData[j].id][selectData[j].lun][i].d;
                }
            ...
            for (let i in selectData) {
                ...
                for (let j = 0; j < lx.length - 1; ++j) {
                    // console.log(lx[j])
                    if (typeof (line_type_width[lx[j].x.toString() + lx[j].y.toString() + lx[j + 1].x.toString() + lx[j + 1].y.toString()]) == 'undefined') {
                        line_type_width[lx[j].x.toString() + lx[j].y.toString() + lx[j + 1].x.toString() + lx[j + 1].y.toString()] = {
                            p: [lx[j], lx[j + 1]],
                            w: 0
                        };
                    }
                    line_type_width[lx[j].x.toString() + lx[j].y.toString() + lx[j + 1].x.toString() + lx[j + 1].y.toString()].w++;
                    line_max = Math.max(line_max, line_type_width[lx[j].x.toString() + lx[j].y.toString() + lx[j + 1].x.toString() + lx[j + 1].y.toString()].w);
                    line_min = Math.min(line_min, line_type_width[lx[j].x.toString() + lx[j].y.toString() + lx[j + 1].x.toString() + lx[j + 1].y.toString()].w);
                }
            }
            ...
            p_g
                ...
                .attr('stroke-opacity', (d, i) => 0.2 * w_scale(type_w[i]));
            ...
            var area_generator = d3.area()
                ...
                .curve(d3.curveMonotoneX)
            for (let i = 0; i < parseInt(move_x); ++i) {
            ...
            drawArea(compare_g, timeLine, max_people_sum, move_y + 450, move_x);
        })
    })
}
...
function drawArea(paint_g, timeLine, max_people, move_y, id) {
