        ...
        d3.csv('data/treatment.csv').then(treat => {
            ...
            for (let i in selectData) {
                ...
                for (let j = 0; j < lx.length - 1; ++j) {
                    // console.log(lx[j])
                    if (typeof (line_type_width[lx[j].x.toString() + lx[j + 1].x.toString()]) == 'undefined') {
                        line_type_width[lx[j].x.toString() + lx[j + 1].x.toString()] = {
                            p: [lx[j], lx[j + 1]],
                            w: 0
                        };
                    }
                    line_type_width[lx[j].x.toString() + lx[j + 1].x.toString()].w++;
                    line_max = Math.max(line_max, line_type_width[lx[j].x.toString() + lx[j + 1].x.toString()].w);
                    line_min = Math.min(line_min, line_type_width[lx[j].x.toString() + lx[j + 1].x.toString()].w);
                }
            }
