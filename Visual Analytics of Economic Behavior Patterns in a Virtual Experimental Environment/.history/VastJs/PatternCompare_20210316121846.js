        ...
        d3.csv('data/treatment.csv').then(treat => {
            ...
            for (let i in selectData) {
                ...
                for (let j = 0; j < lx.length - 1; ++j) {
                    // console.log(lx[j])
                    if (typeof (line_type_width[lx[j].x.toString() + lx[j].y.toString()]) == 'undefined') {
                        line_type_width[lx[j].x.toString() + lx[j].y.toString()] = {
                            x: lx[j].x,
                            y: lx[j].y,
                            w: 0
                        };
                    }
                    line_type_width[lx[j].x.toString() + lx[j].y.toString()].w++;
                    line_max = Math.max(line_max, line_type_width[lx[j].x.toString() + lx[j].y.toString()].w);
                    line_min = Math.min(line_min, line_type_width[lx[j].x.toString() + lx[j].y.toString()].w);
                }
            }
            ...
            for (let i in line_type_width) {
                type_data.push(line_type_width[i]);
            }
            ...
            decisionDiagonal = p_g
                ...
                .attr("fill", "none")
                .attr("stroke", 'steelblue')
                .attr("stroke-width", d => w_scale(d.w))
                .attr('stroke-opacity', 0.3);
