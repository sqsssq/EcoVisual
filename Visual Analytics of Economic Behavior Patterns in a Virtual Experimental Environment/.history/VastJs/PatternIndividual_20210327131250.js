    ...
    d3.csv('data/newdata_for_line.csv').then(wealth_data => {
        ...
        for (let i in personal_data) {
            ...
            g_p.append('circle')
                ...
                .attr('stroke-width', 0.5)
                .on('click', d => {
                    flowerGlyph(pie_data_personal, max_gain);
                });
...
function flowerGlyph(pie_data, max_gain)
{
    const flower_inner = 10;
    const flower_outer = 300;
    // console.log(pie_data, max_gain);
    const flowerScale = d3.scaleLinear()
    .domain([0, max_gain])
    ...
    .attr("transform", `translate(${this.x_scale_wealth(3)}, ${this.y_scale_wealth(300)})`)
    glyph_g.append('rect')
    .attr("x", 0)
    .attr("y", 0)
    .attr("width", 2 * flower_outer)
    .attr("height", 2 * flower_outer)
    .attr("fill", "none")
    .attr("stroke", "black");
    glyph_g
    .append("g")
    .attr("transform", `translate(${(flower_outer)}, ${(flower_outer)}) rotate(-105, 0, 0)`)
