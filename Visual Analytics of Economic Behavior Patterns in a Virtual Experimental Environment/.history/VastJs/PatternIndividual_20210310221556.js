    ...
    d3.csv('data/data_for_line.csv').then(wealth_data => {
        ...
        for (let i = 1; i < 20; ++i) {
            pie_g.append('line')
                ...
                .attr('y1', heightFlower / 12)
                .attr('y2', heightFlower / 12).attr('stroke', 'gray')
        ...
        for (let i in horizon_data) {
            ...
            // console.log(i)
            drawHorizonR(horizon_data[i], max_gain, 15, 9 * parseInt(i) + 3, pie_g);
        }
        pie_g.append('rect').attr('x', 15).attr('y', 3).attr('height', 81).attr('width', 180).attr('fill', 'none').attr('stroke', 'black').attr('stroke-width', 1);
    });
}
