...
function drawWealthLine() {
    d3.csv('data/newdata_for_line.csv').then(wealth_data => {
        ...
        for (let i in box_data) {
            ...
            s_s.push({
                x1: this.x_scale_wealth(parseInt(i) + 1) - 10,
                x2: this.x_scale_wealth(parseInt(i) + 1) - 10,
            ...
            h_s_rect.push({
                x: this.x_scale_wealth(parseInt(i) + 1) - 10,
                y: this.y_scale_wealth(box_data[i]['last_max']),
                width: 20,
                height: Math.abs(this.y_scale_wealth(box_data[i]['last_max']) - this.y_scale_wealth(box_data[i]['last_min']))
            });
        }
        // console.log(h_h);
...
function drawFlower(code, select_round) {
    d3.csv('data/newdata_for_line.csv').then(wealth_data => {
        ...
        d3.select("#wealth_box_g").attr("opacity", 0.3);
