...
function drawWealthLegend(cnt, flag) {
    svgLegend.append('rect')
        ...
        .attr('width', 40)
        .attr('fill', flag == 1 ? DBSCAN_color[clusterSelect] : typeColor[rectSelect]);
        ...
        function brushing() {
            ...
            for (let i = 0; i < scatter_data.length; ++i) {
                ...
                if (x >= x0 && x <= x1 && y >= y0 && y <= y1) {
                    ...
                    selectPeople[scatter_data[i].id + scatter_data[i].l] = typeColor[rectSelect];
                }
            }
            ...
            axisCircle = axis_g.selectAll('#axisCircle')
                ...
                .attr('fill', d => {
                    ...
                    if (selectPeople[d.code + d.lun]) {
                        // console.log(d.code + d.lun);
                        // return typeColor[selectPeople[d.code + d.lun] - 1];
                        return selectPeople[d.code + d.lun];
                    } else return 'gray';
                })
                .attr('stroke', 'none')
                // .attr('fill-opacity', 1)
    ...
    d3.csv('data/newdata_for_line.csv').then((box_data) => {
        ...
        axisCircle = axis_g.selectAll('#axisCircle')
            ...
            .on('click', d => {
                ...
                // selectPeopleCircle(d.nx, d.ny, fill, peo_cir.attr('cx'), peo_cir.attr('cy'), peo_cir.attr('fill'));
                for (let people in d.people_list)
                {
                    selectPeople[people.id + people.lun] = DBSCAN_color[clusterSelect];
                }
