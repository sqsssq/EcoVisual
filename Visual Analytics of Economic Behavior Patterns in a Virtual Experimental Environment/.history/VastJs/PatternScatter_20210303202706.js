...
function DrawScatter() {
    d3.json('data/20210226.json').then((scatter_data) => {
        d3.csv('data/jingshouyi.csv').then((rect_data) => {
            ...
            scatter_g = svgScatter.append('g').attr('width', widthScatter).attr('height', heightScatter / 2);
    ...
    d3.json('data/20210226.json').then((scatter_data) => {
        d3.csv('data/jingshouyi.csv').then((rect_data) => {
            ...
            function brushed() {
                ...
                drawPattern(10 + (countType - 2) * (c_width + 10), 10, selectPattern);
        ...
        d3.csv('data/jingshouyi.csv').then((rect_data) => {
            d3.csv('data/box_calc_rank.csv').then((box_data) => {
                ...
                for (let i = 0; i < box_data.length; ++i) {
                    let temp = {
                        y: parseFloat(box_data[i]['129']) - parseFloat(box_data[i]['19']),
                        x: parseFloat(box_data[i]['19']),
                ...
                console.log(y_array);
                let y_width = yAxisWidth / 2 - 40 - 10;
                let y_start = yAxisWidth / 2 - 40 - 10;
                ...
                for (let i in x_scale_array) {
                    ...
                    axis_g.append('line')
                    .attr('x1', x_scale_array[i].domain()[0])
                    .attr('x2', x_scale_array[i].domain()[0])
                    .attr('y1', 10)
                    .attr('y2', yAxisWidth / 2 - 40)
                    .attr('fill', 'none')
                    .attr('stroke', 'red');
                    axis_g.append('line')
                    .attr('x1', x_scale_array[i].domain()[1])
                    .attr('x2', x_scale_array[i].domain()[1])
                    .attr('y1', 10)
                    .attr('y2', yAxisWidth / 2 - 40)
                    .attr('fill', 'none')
                    .attr('stroke', 'red');
                    axis_g.append('line')
                    .attr('y1', y_scale_array[i].domain()[0])
                    .attr('y2', y_scale_array[i].domain()[0])
                    .attr('x1', 10)
                    .attr('x2', xAxisWidth - 10)
                    .attr('fill', 'none')
                    .attr('stroke', 'red');
                    axis_g.append('line')
                    .attr('y1', y_scale_array[i].domain()[1])
                    .attr('y2', y_scale_array[i].domain()[1])
                    .attr('x1', 10)
                    .attr('x2', xAxisWidth - 10)
                    .attr('fill', 'none')
                    .attr('stroke', 'red');
