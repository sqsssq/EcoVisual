...
function drawFlower(code) {
    d3.csv('data/data_for_line.csv').then(wealth_data => {
        ...
        for (let i = 1; i < 20; ++i) 
        {
            pie_g.append('line').attr('x1', this.x_scale_wealth(i)).attr('x2', this.x_scale_wealth(i + 1)).attr('y1', heightFlower / 12).attr('y2', heightFLower / 12).attr('stroke', 'gray').attr('stroke-width', lineWidthScale(decisionDifferent[i - 1])).attr('fill', 'none');
        }
        this.flowerInner = 5;
        this.flowerOuter = 40;
        ...
        for (let i in horizon_data) {
            ...
            // console.log(i)
            drawHorizonR(horizon_data[i], max_gain, 5, 9 * parseInt(i) + 3, pie_g);
        }
        pie_g.append('rect').attr('x', 5).attr('y', 3).attr('height', 81).attr('width', 200).attr('fill', 'none').attr('stroke', 'black').attr('stroke-width', 1);
    });
}
