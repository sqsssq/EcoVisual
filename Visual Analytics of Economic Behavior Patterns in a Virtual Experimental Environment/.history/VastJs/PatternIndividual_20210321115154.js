...
function drawFlower(code, select_round) {
    d3.csv('data/newdata_for_line.csv').then(wealth_data => {
        ...
        const pie_g = wealth_g.append('g').attr('id', 'pie_g' + (parseInt(select_people_num) + 1).toString());
        pie_g
            // .selectAll('#wg')
            //     .attr('id', 'wg')
        ...
        for (let i in personal_data) {
            ...
            g_p.append('circle')
                ...
                .attr('x', 0)
                .attr('y', 0)
                .attr('fill', i == select_round ? 'blue' : 'white')
                .attr('stroke', 'blue')
                .attr('stroke-width', 0.5);
        }
...
function drawPersonalHorizon(code) {
    d3.csv('data/newdata_for_line.csv').then(wealth_data => {
        ...
        const svgFlower = d3.select('#hMain')
            .append('svg')
            .attr('id', 'horizonSvg' + (parseInt(select_people_num) + 1).toString())
