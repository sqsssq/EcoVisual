    ...
    d3.csv('data/data_for_line.csv').then(data => {
        d3.csv('data/treatment.csv').then(treat => {
            ...
            $(document).ready(function () {
                ...
                <select name="'+ (countType - 1) + '" id="set' + (countType - 1).toString() + '" onclick="console.log(this.value)">\
                ...
                <select name="'+ (countType - 1) + '" id="sex' + (countType - 1).toString() + '" onclick="console.log(this.name)">\
...
function reDrawCurve(p_num)
{
    ...
    DrawCurveLine(selectDataObject[p_num], 10 + (p_num - 1) * (c_width + 10), 10, process, round_num, p_num);
}
