...
function flowerGlyph(pie_data, decisionList, max_gain) {
    ...
    const flower_outer = 150;
    console.log(decisionList);
    const flowerScale = d3.scaleLinear()
        .domain([0, max_gain])
        .range([flower_inner, flower_outer]);
    ...
    const glyph_g = wealth_g.append('g')
        ...
        .attr("transform", `translate(${this.x_scale_wealth(3) - (flower_outer + 20)}, ${this.margin.top})`)
    glyph_g.append('rect')
        .attr("x", 0)
        .attr("y", 0)
        .attr("width", 2 * (flower_outer + 40))
        .attr("height", 2 * (flower_outer + 20))
        .attr("fill", "none")
        .attr("stroke", "black");

        const flower_glyph_g = glyph_g.append("g")
        .attr("transform", `translate(${(flower_outer + 40)}, ${(flower_outer + 20)}) rotate(-105, 0, 0)`)
    ...
    for (let i = 0; i < decisionList.length; ++i) {
        ...
        let x2 = (flower_outer + 10) * Math.cos(rd);
        let y2 = (flower_outer + 10) * Math.sin(rd);

        // console.log(i, count_type_array[i], lineNameLegend[i][count_type_array[i] - 1])

        glyph_g.append("g")
        .attr("transform", `translate(${(flower_outer + 40)}, ${(flower_outer + 20)})`).append('text')
            .attr('x', x2)
            .attr('y', y2)
            .attr('text-anchor', 'middle')
            .attr('font-family', 'Georgia')
