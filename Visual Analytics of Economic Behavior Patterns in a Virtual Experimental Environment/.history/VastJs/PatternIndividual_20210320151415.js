...
function drawPersonalHorizon(code) {
    d3.csv('data/newdata_for_line.csv').then(wealth_data => {
        ...
        pie_g.append('rect').attr('x', 20).attr('y', 11).attr('height', (heightFlower - 24) / 4 - 20).attr('width', widthHorizonMain - 30 - 10).attr('fill', 'none').attr('stroke', 'black').attr('stroke-width', 1);
    });
}
...
function drawHorizonR(roundCount, max_num, move_x, move_y, compare_g, id) {
    ...
    let x_scale = d3.scaleLinear()
        ...
        .range([0, widthHorizonMain - 30 - 10]);
