    ...
    d3.csv('data/data_for_line.csv').then(data => {
        d3.csv('data/treatment.csv').then(treat => {
            ...
            $(document).ready(function () {
                ...
                <select name="select2" id="set' + (countType - 1).toString() + '" onclick="console.log(this.value)">\
...
// let curveLine_g = 0;
function reDrawCurve(selectData, move_x, move_y, p_num)
{
    ...
    DrawCurveLine(selectData, move_x, move_y, process, round_num, p_num);
}
...
function DrawCurveLine(selectData, move_x, move_y, process, round_num, p_num) {
    ...
    for (let i in selectData) {
        ...
        for (let j = parseInt(selectData[i].lun); j > (parseInt(selectData[i].lun) - round_num > 0 ? parseInt(selectData[i].lun) - round_num : 0); --j) {
            ...
            if (process == 10)
            {
                k = parseFloat(selectData[i]['start' + (j).toString()]);
            }
            if (process == 11)
            {
                k = parseFloat(selectData[i]['net' + (j).toString()]);
            }
            if (process == 12)
            {
                k = parseFloat(selectData[i]['end' + (j).toString()]);
            }
            // console.log(k);
