...
function DrawAxis(scatter_data, flag) {
    d3.csv('data/newdata_for_line.csv').then((box_data) => {
        ...
        var xScale = d3.scaleLog()
            .base(2)
            .domain([x_min - x_min + 1, x_max - x_min + 1])
            .range([10, xAxisWidth - 10]);
        // console.log(xScale.domain());
        var yScale = d3.scaleLog()
            .base(2)
            .domain([y_max - y_min + 1, y_min - y_min + 1])
            .range([10, yAxisWidth / 2 - 40]);

        axis_g.append("g")
            .attr("transform", "translate(" + 0 + "," + (0 + yScale(-y_min + 1)) + ")")
            .call(d3.axisBottom(xScale).ticks(10))
        // // .append("text")
        // // .attr("font-family", "Georgia")
        // // .attr('font-size', 15)
        ...
        // // .text("round");
        axis_g.append("g")
            .attr("transform", "translate(" + xScale(-x_min + 1) + "," + (0) + ")")
            .call(d3.axisLeft(yScale).ticks(10))
        // // .append("text")
        // // .attr("font-family", "Georgia")
        // // .attr('font-size', 15)
        ...
        axisCircle = axis_g.selectAll('#axisCircle')
            ...
            .attr('id', d => 'axis_cir_' + d.code)
            .attr('cx', d => {
                d.nx = xScale(d.x - x_min + 1);
                return d.nx;
            })
            // .attr('cy', d => yScale(d.y))
            .attr('cy', d => {
                d.ny = yScale(d.y - y_min + 1);
                return d.ny;
            })
            // .attr('r', 2)
            .attr('r', d => {
