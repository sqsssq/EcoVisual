...
function drawPattern(move_x, move_y, selectData) {
    d3.csv('data/data_for_line.csv').then(data => {
        d3.csv('data/treatment.csv').then(treat => {
            ...
            for (let i = 0; i < 10; ++i) {
                ...
                for (let j = 1; j <= lineType[i]; ++j) {
                    p_g.append('circle')
                        ...
                        .attr('cy', type_scale(j) * Math.sin(rd) + 150)
                        .attr('r', 1)
                        .attr('fill', 'red')
                }
            }
            ...
            for (let i in selectData) {
                ...
                for (let j = 0; j < 10; ++j) {
                    ...
                    lx.push({
                        ...
                        y: peopleHistory[selectData[i].id][selectData[i].lun][j].dy + 150
            ...
            drawHorizon(roundCount, 'Round Number', move_x, move_y + 300);
