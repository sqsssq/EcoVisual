...
function drawPattern(move_x, move_y, selectData, flag) {
    d3.csv('data/newdata_for_line.csv').then(data => {
        d3.csv('data/treatment.csv').then(treat => {
            ...
            p_g.append('rect').attr('x', 0).attr('y', 0).attr('width', c_width).attr('height', 35).attr("fill", "rgb(210, 229, 236)");
            p_g.append('text').text("Pattern " + (countType - 1).toString()).attr('dx', '0.5em').attr('dy', '1.2em').attr('font-family', 'STHeiti').attr('font-size', 20).attr('font-weight', 'bold');
