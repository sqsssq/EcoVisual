...
function drawWealthLine() {
    d3.csv('data/newdata_for_line.csv').then(wealth_data => {
        ...
        for (let i in box_data)
        {
            ...
            s_s.push({
                x1: this.x_scale_wealth(parseInt(i) + 1) - 5,
                x2: this.x_scale_wealth(parseInt(i) + 1) - 5,
                y1: this.y_scale_wealth(box_data[i]['last_min']),
                y2: this.y_scale_wealth(box_data[i]['last_max'])
            });
            s_s.push({
                x1: this.x_scale_wealth(parseInt(i) + 1) + 5,
                x2: this.x_scale_wealth(parseInt(i) + 1) + 5,
            ...
            s_s.push({
                x1: this.x_scale_wealth(parseInt(i) + 1),
                x2: this.x_scale_wealth(parseInt(i) + 1),
                y1: this.y_scale_wealth(box_data[i]['max']),
                y2: this.y_scale_wealth(box_data[i]['last_max'])
            });
            s_s.push({
                x1: this.x_scale_wealth(parseInt(i) + 1),
                x2: this.x_scale_wealth(parseInt(i) + 1),
                y1: this.y_scale_wealth(box_data[i]['last_min']),
                y2: this.y_scale_wealth(box_data[i]['min'])
            });
            h_s_rect.push({
                x: this.x_scale_wealth(parseInt(i) + 1) - 5,
                y: this.y_scale_wealth(box_data[i]['last_max']),
                width: 10,
                height: this.y_scale_wealth(box_data[i]['last_max'] - box_data[i]['last_min'])
            });
            console.log(box_data[i]['last_max'] - box_data[i]['last_min']);
        }

        let box_g = wealth_g.append('g')
        .attr('id', 'wealth_box_g')
        
        box_g.selectAll("#h_s_rect")
        .attr("id", "h_s_rect")
        .data(h_s_rect)
        .enter()
        .append("rect")
...
function drawFlower(code, select_round) {
    d3.csv('data/newdata_for_line.csv').then(wealth_data => {
        ...
        for (let i in personal_data) {
            ...
            const g_p = pie_g.selectAll("#flower_x" + (parseInt(select_people_num) - 1).toString())
                .attr('id', 'flower_x' + (parseInt(select_people_num) - 1).toString())
                .data(pie_data_personal)
                .enter()
                .append('g').attr("transform", "translate(" + this.x_scale_wealth(parseInt(i) + 1) + "," + (this.y_scale_wealth(wealthList[i].y)) + ") rotate(-97.5, " + 0 + "," + 0 + ")")
            g_p.append('path')
...
function drawPersonalHorizon(code, cnt) {
    d3.csv('data/newdata_for_line.csv').then(wealth_data => {
        const personal_data = new Array();
        for (let i in wealth_data) {
            if (wealth_data[i].code == code) {
        ...
        for (let i in horizon_data) {
            // if (parseInt(i) == 9) break;
            // console.log(i)
            drawHorizonR(horizon_data[i], max_gain, 20, ((heightFlower - 24) / 4 - 20) / 12 * parseInt(i) + 10, pie_g, select_people_num - 1);
        }
        pie_g.append('rect').attr('x', 20).attr('y', 11).attr('height', (heightFlower - 24) / 4 - 20).attr('width', widthHorizonMain - 30 - 15).attr('fill', 'none').attr('stroke', 'black').attr('stroke-width', 1);
    });
}
