                ...
                d3.csv("data/treatment.csv").then((treatment) => {
                    ...
                    for (let i = 0; i < data.length; ++i) {
                        if (isNaN(parseInt(name[data[i].code])))
                            continue;
                        let arc = d3.arc()
                            .innerRadius(15 + wScale(parseFloat(data[i]['129'])))
                            .outerRadius(17 + wScale(parseFloat(data[i]['129'])));
                        ...
                        svg
                            ...
                            .enter()
                            .append('g')
                            .attr('transform', 'translate(' + (60 + parseInt(data[i].biao) * 55) + ',' + (50 * (name[data[i].code] + 1) - 3) + ')')
                            .append('path')
                            .attr('d', d => {
                                // console.log(d);
                                return arc(d);
