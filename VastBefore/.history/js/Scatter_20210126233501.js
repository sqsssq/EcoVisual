...
function ScatterPaint_gain_loss() {
    d3.csv('data/fpmwithtsne.csv').then((coor) => {
        d3.csv('data/box_calc.csv').then((rectdata) => {
            ...
            for (var i in coor) {
                max_x = Math.max(max_x, parseFloat(coor[i][0]))
                max_y = Math.max(max_y, parseFloat(coor[i][1]))
                min_x = Math.min(min_x, parseFloat(coor[i][0]))
                min_y = Math.min(min_y, parseFloat(coor[i][1]))
            }
            ...
            tcircle = ssvg.append('g').selectAll("circle")
                ...
                .attr("cx", function (d) {
                    console.log(d);
                    return Math.round(xScale(parseFloat(d[0]), 5));
                })
                .attr("cy", function (d) {
                    return Math.round(yScale(parseFloat(d[1]), 5));
                })
                .attr("r", 2)
                .attr('stroke', (d, i) => {
                    //if (d.l == num)
