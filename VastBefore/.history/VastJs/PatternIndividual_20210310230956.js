...
function drawFlower(code) {
    d3.csv('data/data_for_line.csv').then(wealth_data => {
        ...
        for (let i in horizon_data) {
            ...
            // console.log(i)
            drawHorizonR(horizon_data[i], max_gain, 60, 9 * parseInt(i) + 3, pie_g);
        }
        pie_g.append('rect').attr('x', 60).attr('y', 3).attr('height', 81).attr('width', 180).attr('fill', 'none').attr('stroke', 'black').attr('stroke-width', 1);
    });
}
