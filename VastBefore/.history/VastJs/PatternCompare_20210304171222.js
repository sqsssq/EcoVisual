    ...
    d3.csv('data/data_for_line.csv').then(data => {
        d3.csv('data/treatment.csv').then(treat => {
            ...
            for (let i = 0; i < data.length; ++i) {
                ...
                peopleHistory[data[i].code]['end' + data[i].biao] = parseFloat(data[i][139]);
                peopleHistory[data[i].code]['net' + data[i].biao] = parseFloat(data[i][139]) - parseFloat(data[i][19]);
            ...
            $(document).ready(function () {
                ...
                <select name="select2" id="se' + (countType - 1).toString() + '" onclick="console.log(this.value)">\
                    <option value="1">Initial</option>\
                    <option value="2">Work</option>\
                    <option value="3">Health</option>\
                    <option value="4">Insurance</option>\
                    <option value="5">Loan</option>\
                    <option value="6" selected>Investment</option>\
                    <option value="7">Risk</option>\
                    <option value="8">Disaster</option>\
                    <option value="9">Lottery</option>\
                    <option value="10">Ill</option>\
                    <option value="11">Unemployed</option>\
                    <option value="12">Net yield</option>\
                    <option value="13">Total</option>\
                </select></div>');
                $("#ganttView").append('<div style="position: absolute; top: 355px; left: ' + (210 + move_x) + 'px; font-family: Georgia;">Round: \
                <select name="select1" id="sex' + (countType - 1).toString() + '" onclick="console.log(this.id)">\
...
function DrawCurveLine(selectData, move_x, move_y, process, round_num) {
    ...
    let curveLine_g = compare_g.append('g').attr('id', 'a1').attr('transform', 'translate(' + move_x + ',' + (move_y) + ')');
    let max_y = -9999;
    let min_y = 9999;
