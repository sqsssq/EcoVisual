...
function drawHorizon(selectData, move_x, move_y) {
    let roundCount = new Object();
    for (let i = 1; i <= 20; ++i) {
        roundCount[i] = 0;
    }
    let max_people = 0;
    for (let i in selectData) {
        roundCount[parseInt(selectData[i].lun)]++;
        max_people = Math.max(max_people, roundCount[parseInt(selectData[i].lun)])
    }
    let horizon_g = compare_g.append('g').attr('transform', 'translate(' + move_x + ',' + (move_y + 270) + ')');
