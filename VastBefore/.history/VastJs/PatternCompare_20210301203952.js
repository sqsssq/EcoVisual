...
function drawPattern(move_x, move_y, selectData) {
    d3.csv('data/data_for_line.csv').then(data => {
        ...
        for (let i = 0; i < data.length; ++i) {
            ...
            peopleHistory[data[i].code][data[i].lun] = new Array();
            for (let j = 1; j < 11; ++j) {
                ...
                if (c < -200) c = -200;
                if (c > 200) c = 200;
                peopleHistory[data[i].code][data[i].lun].push({
                    v: c
                });
        ...
        for (let i = 0; i < 10; ++i) {
            ...
            if (i != 9) {
                ...
                let line_scale_2 = d3.scaleLinear()
                    .domain([-max_axis[i + 1], max_axis[i + 1]])
                    .range([40, 135]);

                for (let j in selectData) {
                    let r1 = line_scale_1(peopleHistory[selectData[j].id][selectData[j].lun][i]);
                    let r2 = line_scale_2(peopleHistory[selectData[j].id][selectData[j].lun][i + 1]);
                    let tmp = {
                        source: {
                            x: r1 * Math.cos(rd),
