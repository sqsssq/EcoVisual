...
function DrawAxis() {
    d3.csv('data/box_calc_rank.csv').then((box_data) => {
        ...
        for (let i in x_scale_array) {
            const flagX = Math.abs((x_scale_array[i].range()[1]) - (x_scale_array[i].range()[0])) / x_width > 0.1 ? 2 : 0;
            const flagY = Math.abs((y_scale_array[i].range()[1]) - (y_scale_array[i].range()[0])) / y_width > 0.1 ? 2 : 0;
            console.log(flagX, flagY);
            axis_g.append("g")
                .attr("transform", "translate(" + 0 + "," + (0 + y_scale_array[y_axis_type](0)) + ")")
                .attr('class', 'axis')
                .call(d3.axisBottom(x_scale_array[i]).ticks(flagX))
            axis_g.append("g")
                .attr("transform", "translate(" + x_scale_array[x_axis_type](0) + "," + (0) + ")")
                .attr('class', 'axis')
                .call(d3.axisLeft(y_scale_array[i]).ticks(flagY))
        }
