...
function drawPattern(move_x, move_y, selectData) {
    d3.csv('data/data_for_line.csv').then(data => {
        ...
        $(document).ready(function () {
            $("#ganttView").append('<select transform="translate(' + move_x + ',' + move_y + ')" name="select1" id="se" onclick="console.log(this.value)"><option value="1">start</option><option value="2">Work</option><option value="3">health</option><option value="6" selected>Investment</option><option value="1">start</option></select>');
        })
    })
}
