    ...
    d3.csv('data/data_for_line.csv').then(data => {
        d3.csv('data/treatment.csv').then(treat => {
            ...
            for (let i in selectData) {
                ...
                treatCount[parseInt(selectData[i].treatData) - 1]++;
                console.log(parseInt(selectData[i].treatData))
            }

            drawHorizon(roundCount, 'Round Number', move_x, move_y + 160);
            drawHorizon(treatCount, 'Treatment', move_x, move_y + 200);
        })
    })
}
...
function DrawCurveLine(selectData, move_x, move_y, process, round_num) {
    ...
    let curveLine_g = compare_g.append('g').attr('transform', 'translate(' + move_x + ',' + (move_y + 280) + ')');
    let max_y = -9999;
    let min_y = 9999;
