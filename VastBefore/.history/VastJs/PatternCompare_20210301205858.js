    ...
    d3.csv('data/data_for_line.csv').then(data => {
        ...
        for (let i = 0; i < 10; ++i) {
            ...
            for (let j in selectData) {
                ...
                peopleHistory[selectData[j].id][selectData[j].lun][i].x = (x).toFixed(2);
                peopleHistory[selectData[j].id][selectData[j].lun][i].y = (y).toFixed(2);
            }
        ...
        for (let i in selectData) {
            for (let j = 0; j < 9; ++j) {
                line_array.push({
                    source: {
                        x: peopleHistory[selectData[i].id][selectData[i].lun][j].x,
                        y: peopleHistory[selectData[i].id][selectData[i].lun][j].y
                    },
                    target: {
                        x: peopleHistory[selectData[i].id][selectData[i].lun][j + 1].x,
                        y: peopleHistory[selectData[i].id][selectData[i].lun][j + 1].y
                    }
                })
            }
        }
        ...
        let decisionDiagonal = p_g.append("g")
