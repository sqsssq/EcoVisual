...
function DrawLine(data3, isSelect) {
    d3.csv("data/box_calc_rank.csv").then((data) => {
                ...
                for (let k = 2; k <= 11; ++k) {
                    if (typeof(connectData[data3[i].people[j][k - 1] + "-" + data3[i].people[j][k]]) == 'undefined') {
                        connectData[data3[i].people[j][k - 1] + "-" + data3[i].people[j][k]] = {
                            ...
                            target: parseInt(data3[i].people[j][k]),
                            val: 0,
                            type: parseInt(i)
                        }
                    }
        ...
        for (let i in connectData) {
            ...
            console.log("target", connectData[i].source);
            let h = widthMatrix;
            let x1 = (pointPaint[connectData[i].source].decision + 1) * h / type[pointPaint[connectData[i].source].step];
            let x2 = (pointPaint[connectData[i].target].decision + 1) * h / type[pointPaint[connectData[i].target].step];
