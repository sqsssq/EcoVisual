...
var LinePaint_2 = function (path, dia, color, num) {
    ...
    var line_linearx = d3.scale.linear()
        .domain([0, lg_line])
        .range([0, 70])
    Line_Name = new Array();
    Line_Name.push(Rect_g.selectAll(".line_r")
    ...
    d3.csv("data/box.csv", function (d1) {
        ...
        d3.json('data/ScatterF.json', function (coork) {
            d3.csv("data/box_calc.csv", function (RectInData) {
                ...
                Rect_g = svg.append('g').attr("transform", "translate(" + 0 + "," + -60 + ")")
                ...
                for (var i in RectInnerData) {
                    for (var j = 1; j <= 12; ++j) {
                        RectInnerData[i][j * 10 + 7] = title_tip_symbol[parseInt(j)][parseInt(RectInnerData[i][j])]
                        RectInnerData[i][j * 10 + 8] = title_tip[parseInt(j)][parseInt(RectInnerData[i][j])]
                    }
                }
                // console.log(RectInnerData)
                let RectOuterData = []
                ...
                for (var i in RectInnerData) {
                    RectOuterData[2][RectInnerData[i][1]]['member'].push(RectInnerData[i])
                    RectOuterData[2][RectInnerData[i][1]].val += parseInt(code_Num[RectInnerData[i].code])
                }
                for (var i in RectOuterData[2]) {
                    RectOuterData[2][i].val /= RectOuterData[2][i]["member"].length
                }
                // for (var t = 1; t <= 20; ++t) {
                for (var k = 3; k <= 13; ++k) {
                    // console.log(k)
