    ...
    d3.json('data/DT/sum2.json', function (sdata) {
        d3.csv('data/box_calc.csv', function (bdata) {
            ...
            for (let p = 1; p <= 20; ++p) {
                ...
                for (let j in Cir_2) {
                    Dec_2[Cir_2[j].Decision[Cir_2[j].main]] += Cir_2[j].member.length;
                ...
                for (let j in Cir_3) {
                    Dec_3[Cir_3[j].Decision[Cir_3[j].main]] += Cir_3[j].member.length;
                    if (typeof (cal_2[parseInt(Cir_3[j].mid) * 100 + parseInt(Cir_3[j].main)]) == 'undefined') {
                        cal_1[parseInt(Cir_3[j].mid) * 100 + parseInt(Cir_3[j].main)] = 0;
                    }
            ...
            var scale_2 = d3.scale.linear()
                ...
                .range([0, 270]);
            console.log(Sum(Dec_2, 0, Dec_2.length))
            R_svg.selectAll('#DRect')
                .attr('id', 'DRect')
                .data(Dec_2)
                .enter()
