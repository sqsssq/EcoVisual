    ...
    d3.json('data/DK/sum2.json', function (sdata) {
        d3.csv('data/box_calc.csv', function (bdata) {
            ...
            for (let i in PieData) {
                maxPeo = Math.max(PieData[i].member.length, maxPeo);
                minPeo = Math.min(PieData[i].member.length, maxPeo);
            }
            ...
            for (let i in PieData) {
                var arc1 = d3.svg.arc()
                    .innerRadius(scale_x(PieData[i].member.length))
                    .outerRadius(scale_x(PieData[i].member.length))
                let StartAngle = 0;
                var arc_data1;
                if (PieData[i].l != 0) {
                    arc_data1 = {
                        startAngle: StartAngle * Math.PI * 2,
                        endAngle: (StartAngle + PieData[i].l / PieData.member.length) * 2 * Math.PI
                    }
                    StartAngle += PieData[i].l / PieData.member.length;
                    R_svg.append('g')
                        .append('path')
