...
d3.csv('data/box_calc.csv', function (Ice_d) {
    ...
    for (var k = 0; k < 11; ++k) {
        ...
        p_g.selectAll('#linein')
            ...
            .attr('y2', d => {
                if (Math.log10(Math.abs(parseFloat(d[129]) - parseFloat(d[19]))) <= 0)
                return r[k].n * height_ice / 4 - height_ice / 8
                if (k == 0)
                    return height_ice / 8 - line_scale(Math.log10(Math.abs(parseFloat(d[129]) - parseFloat(d[19])))) / 2
                return r[k].n * height_ice / 4 - height_ice / 8 - line_scale(Math.log10(Math.abs(parseFloat(d[129]) - parseFloat(d[19])))) / 2
            })
            .attr('fill', 'none')
            .attr('stroke', d => {
