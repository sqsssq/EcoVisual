...
var DecisionRectStep = 90;
var DecisionRectBei = 0.01
        ...
        if (Decision == -1 || Decision.length == 0) {
            ...
            for (let i in RectInnerData) {
                ...
                for (let j = 0; j < Decision.length - 1; ++j) {
                    if (typeof (p[(Decision[j]).toString() + RectInnerData[i][Decision[j]].toString() + (Decision[j + 1]).toString() + RectInnerData[i][(Decision[j + 1])].toString()]) == 'undefined')
                        p[(Decision[j]).toString() + RectInnerData[i][Decision[j]].toString() + (Decision[j + 1]).toString() + RectInnerData[i][(Decision[j + 1])].toString()] = 1;
                    else
                        p[(Decision[j]).toString() + RectInnerData[i][Decision[j]].toString() + (Decision[j + 1]).toString() + RectInnerData[i][(Decision[j + 1])].toString()]++;
                }
            }
            let dia_path = new Array();
            // console.log(p)
            ...
            for (var i in p) {
                ...
                b = {
                    source: {
                        x: 15 + RectX[i[0] + i[1]].start * 6080 * DecisionRectBei / RectX[i[0] + i[1]].s_sum + RectX[i[0] + i[1]].n * tt + (dp + p[i] / 2) * DecisionRectBei,
                        y: 70 + CCPP[parseInt(i[0])] * 32 * 9 / (Decision.length - 1) + 10
                    },
                    target: {
                        x: 15 + RectX[i[2] + i[3]].start * 6080 * DecisionRectBei / RectX[i[2] + i[3]].s_sum + RectX[i[2] + i[3]].n * ttt + (dpp + p[i] / 2) * DecisionRectBei,
                        y: 70 + CCPP[parseInt(i[2])] * 32 * 9 / (Decision.length - 1)
                    }
                }
