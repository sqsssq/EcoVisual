...
function PaintBackground() {
    ...
    d3.csv('data/box_calcr.csv', function (data) {
        ...
        for (let i = 1; i <= 20; ++i) {
            ...
            Box_Plot[i]['midH'] = Box_Plot_data[i][Box_Plot_data[i].length * 3 / 4];
            Box_Plot[i]['mid'] = Box_Plot_data[i][Box_Plot_data[i].length / 2];
            Box_Plot[i]['midL'] = Box_Plot_data[i][Box_Plot_data[i].length * 1 / 4];
        ...
        for (let i = 1; i <= 20; ++i) {
            ...
            for (let j in Box_Plot[i]) {
                ldata.push({
                    x1: p_xscale(i) - 5,
                    x2: p_xscale(i) + 5,
                    y1: p_yscale(Box_Plot),
                    y2: p_yscale(Box_Plot),
                })
            }
