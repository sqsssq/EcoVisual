...
function FinaceRect(num) {
    ...
    d3.csv("data/box_calc.csv", function (data) {
        ...
        var l_x_scale = d3.scale.linear()
            .domain([0, 303])
            .range([10, widthLine - 5])
        var xAxis = d3.svg.axis().scale(l_x_scale).ticks(20).tickFormat(d3.format("d")).orient("bottom");
        var yAxis = d3.svg.axis().scale(l_rect_scale).ticks(0).tickFormat(d3.format("d")).orient("left"); //添加一个g用于放x轴
        rg_line.append("g")
            .attr("class", "axis")
        ...
        var rect_line = rg_line.selectAll('#rlll')
            ...
            .attr('y', (d, i) => {
                ...
                return 160 - l_rect_scale(Math.abs(parseFloat(d['129'])));
            })
            .attr('height', d => {
