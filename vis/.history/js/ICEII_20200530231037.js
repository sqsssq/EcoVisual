...
function DrawIceRectNum(num) {
    ...
    d3.csv('data/box_calcr.csv', function (Ice_d_k) {
        ...
        for (let i = 0; i < 33; ++i) {
            ...
            r[i].shannon = -sum;
            max = Math.max(max, r[i].shannon);
            min = Math.min(min, r[i].shannon);
        }

        let colore = d3.interpolate('white', color[3]);
        var color_scale = d3.scale.linear()
            .domain([0, max])
            .range([0, 1])
        ...
        p_g.selectAll('#rr_')
            ...
            .attr('stroke-width', 1)
            .attr('fill', d => {
                return  colore(color_scale(d.shannon));
            })
        ...
        for (var k = 0; k < 33; ++k) {
            ...
            p_g.selectAll('#linein')
                ...
                .attr('x1', (d, i) => {
                    ...
                    // if (r[k].n == 4) cnt += r[3].member.length
                    // console.log(i / 10 + cnt / 10 + r[k].num * 1)
                    return (i * 2 + cnt * width_ice / 304 + r[k].num * 1);
                })
                .attr('y1', d => {
                    if (k == 0)
                        return height_ice / (lxBei)
