...
function SelectDecision(num) {
    let KRect = RectLine.append('g')
        ...
        .on('click', function () {
            ...
            DecisionList = temp;
            PaintDecisionRect(DecisionList);
        })
    ...
    PaintDecisionLine(DecisionList);
    PaintDecisionRect(DecisionList);
}
...
var LinePaintDecision = function (dia, pValue) {
    ...
    LineName = DecisionLine.selectAll('#dia_g')
        .attr('id', 'dia_g')
        .data(dia)
        .enter()
        .append('g')
    ...
    d3.csv("data/box_calc.csv", function (RectInnerData) {
        ...
        if (Decision == -1 || Decision.length == 0) {
            ...
            for (var i in p) {
                ...
                b = {
                    source: {
                        x: 15 + RectX[i[0] + i[1]].start * DecisionRectBei + RectX[i[0] + i[1]].n * tt + (dp + p[i] / 2) * DecisionRectBei,
                        y: 70 + RectX[i[0] + i[1]].x * 32 + 10
                    },
                    target: {
                        x: 15 + RectX[i[2] + i[3]].start * DecisionRectBei + RectX[i[2] + i[3]].n * ttt + (dpp + p[i] / 2) * DecisionRectBei,
            ...
            for (let i in RectInnerData) {
                ...
                for (let j = 0; j < Decision.length - 1; ++j) {
                    if (typeof (p[(Decision[j] - 1).toString() + RectInnerData[i][Decision[j]].toString() + (Decision[j + 1]).toString() + RectInnerData[i][(Decision[j + 1])].toString()]) == 'undefined')
                    p[(Decision[j] - 1).toString() + RectInnerData[i][Decision[j]].toString() + (Decision[j + 1]).toString() + RectInnerData[i][(Decision[j + 1])].toString()] = 1;
                    else
                    p[(Decision[j] - 1).toString() + RectInnerData[i][Decision[j]].toString() + (Decision[j + 1]).toString() + RectInnerData[i][(Decision[j + 1])].toString()]++;
                }
            }
            let dia_path = new Array();
            console.log(p)
            ...
            for (var i in p) {
                ...
                console.log(RectX[i[0] + i[1]].x)
                console.log(RectX[i[2] + i[3]].x)
                b = {
                    source: {
                        x: 15 + RectX[i[0] + i[1]].start * DecisionRectBei + RectX[i[0] + i[1]].n * tt + (dp + p[i] / 2) * DecisionRectBei,
                        y: 70 + CCPP[RectX[i[0] + i[1]].x + 1] * 32 * 9 / (Decision.length - 1) + 10
                    },
                    target: {
                        x: 15 + RectX[i[2] + i[3]].start * DecisionRectBei + RectX[i[2] + i[3]].n * ttt + (dpp + p[i] / 2) * DecisionRectBei,
                        y: 70 + CCPP[RectX[i[2] + i[3]].x + 1] * 32 * 9 / (Decision.length - 1)
                    }
                }
