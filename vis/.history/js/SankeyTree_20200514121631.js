        ...
        if (Decision == -1 || Decision.length == 0) {
            ...
            for (let i in RectInnerData) {
                ...
                for (let j = 0; j < Decision.length - 1; ++j) {
                    if (typeof (p[(Decision[j] - 1).toString() + RectInnerData[i][Decision[j]].toString() + (Decision[j]).toString() + RectInnerData[i][(Decision[j + 1])].toString()]) == 'undefined')
                    p[(Decision[j] - 1).toString() + RectInnerData[i][Decision[j]].toString() + (Decision[j]).toString() + RectInnerData[i][(Decision[j + 1])].toString()] = 1;
                    else
                    p[(Decision[j] - 1).toString() + RectInnerData[i][Decision[j]].toString() + (Decision[j]).toString() + RectInnerData[i][(Decision[j + 1])].toString()]++;
                }
            }
            let dia_path = new Array();
            // console.log(p)
            ...
            for (var i in p) {
                ...
                console.log(70 + CCPP[RectX[i[0] + i[1]].x] * 32 * 9 / (Decision.length - 1) + 10)
                b = {
                    source: {
                        x: 15 + RectX[i[0] + i[1]].start * DecisionRectBei + RectX[i[0] + i[1]].n * tt + (dp + p[i] / 2) * DecisionRectBei,
                        y: 70 + CCPP[RectX[i[0] + i[1]].x] * 32 * 9 / (Decision.length - 1) + 10
                    },
                    target: {
                        x: 15 + RectX[i[2] + i[3]].start * DecisionRectBei + RectX[i[2] + i[3]].n * ttt + (dpp + p[i] / 2) * DecisionRectBei,
                        y: 70 + CCPP[RectX[i[2] + i[3]].x] * 32 * 9 / (Decision.length - 1)
                    }
                }
