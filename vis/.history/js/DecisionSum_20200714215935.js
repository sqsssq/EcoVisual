    ...
    d3.json('data/DK/sum2.json', function (sdata) {
        d3.csv('data/box_calc.csv', function (bdata) {
            ...
            for (let i = 0; i < 5; ++i) {
                ...
                if (PieData[i].low != 0) {
                    ...
                    R_svg.append('g')
                        ...
                        .attr('transform', 'translate(' + 550 + ',' + (40 + 50 * i) + ')')
                        // .attr('stroke', '	#D3D3D3')
            ...
            for (let i = 0; i < 5; ++i) {
                ...
                let mark = 0;
                for (let j in cal_2) {
                    let x = parseInt(j % 10000 / 100);
                    let y = parseInt(j % 100);
                    if (x < PieData[i].x3 || (x == parseInt(PieData[i].x3) && y < parseInt(PieData[i].y3))) {
                        kn += cal_2[j].member.length;
                    }
                    if (x == parseInt(PieData[i].x3) && y == parseInt(PieData[i].y3)) {
                        mark = j;
                    }
                }
                dia_path.push({
                    source: {
                        x: krscale(kn) + krscale(cal_2[mark]) / 2 + 10 + 10,
