...
function ScatterPaint_gain_loss(coor, p, num, pf, pdata) {
    // PP()
    // for (var i in coor) {
    ...
    for (var i in Y) {
        max_x = Math.max(max_x, parseFloat(Y[i][0]))
        max_y = Math.max(max_y, parseFloat(Y[i][0]))
        min_x = Math.min(min_x, parseFloat(Y[i][1]))
        min_y = Math.min(min_y, parseFloat(Y[i][1]))
    }
