...
function PaintRactIn() {
    ...
    d3.json('data/DK/sum2.json', function (sdata) {
        d3.csv('data/box_calc.csv', function (bdata) {
            ...
            for (let i = 0; i < 5; ++i) {
                var arc1 = d3.svg.arc()
                    .innerRadius(scale_x(PieData[i].len))
                ...
                if (PieData[i].l != 0) {
                    arc_data1 = {
                        ...
                        endAngle: (StartAngle + PieData[i].l / PieData[i].len) * 2 * Math.PI
                    }
                    console.log(arc_data1)
                    StartAngle += PieData[i].l / PieData[i].len;
                    R_svg.append('g')
                        .append('path')
