...
function PaintRactIn() {
    ...
    d3.json('data/DK/sum2.json', function (sdata) {
        d3.csv('data/box_calc.csv', function (bdata) {
                ...
                for (let knumr in pdata) {
                    ...
                    for (let j in Cir_2) {
                        ...
                        if (!flag) {
                            // console.log(t12)
                            // t12 += 1
                            if (parseInt(Cir_2[j].Decision[2]) == 1) {
                                console.log(Cir_2[j])
                            }
            ...
            R_svg.selectAll('#DRectt2')
                .attr('id', 'DRectt2')
                .data(dx_2)
                .enter()
                .append('text')
                .attr('x', 30 + RectInStep + RectInWidth / 2)
                .attr('y', (d, i) => {
                    if (d != 0) {
                        let ks = Sum(dx_2, 0, i);
                        return scale_2(ks) + 10;
            ...
            R_svg.selectAll('#DRectt2')
                ...
                .enter()
                .append('text')
                .attr('x', 30 + 2 * RectInStep + RectInWidth / 2)
                .attr('y', (d, i) => {
                    if (d != 0) {
                        let ks = Sum(dx_3, 0, i);
                        return scale_2(ks) + 10 + 10;
                    }
                    // console.log(ks);
                })
                .attr('font-size', 9)
            ...
            for (let i = 0; i < 5; ++i) {
                ...
                if (PieData[i].low != 0) {
                    ...
                    R_svg.append('g')
                        ...
                        .attr('transform', 'translate(' + 580 + ',' + (40 + 50 * i) + ')')
                        // .attr('stroke', '	#D3D3D3')
            ...
            for (let i = 0; i < 5; ++i) {
                ...
                d_p.push({
                    source: {
                        x: krscale(kn) + krscale(dx_3[mark].val) / 2 + 10 + 10,
                        y: 30 + RectInWidth + RectInStep * 2
                    },
                    target: {
