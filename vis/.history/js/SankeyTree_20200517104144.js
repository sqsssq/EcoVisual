...
function PaintDecisionLine(Decision, people) {
    ...
    d3.csv("data/box_calc.csv", function (RectInData) {
        ...
        if (Decision == -1 || Decision.length == 0) {
            ...
            for (let i in RectOuterData) {
                // console.log(RectInnerData[i])
                for (let j = 1; j < 10; ++j) {
                    if (typeof (p[(j - 1).toString() + RectOuterData[i][j].toString() + (j).toString() + RectOuterData[i][j + 1].toString()]) == 'undefined')
                        p[(j - 1).toString() + RectOuterData[i][j].toString() + (j).toString() + RectOuterData[i][j + 1].toString()] = 1;
                    else
                        p[(j - 1).toString() + RectOuterData[i][j].toString() + (j).toString() + RectOuterData[i][j + 1].toString()]++;
                }
            }
