...
function DrawPattern(pattern_select) {
    ...
    let yScale2 = d3.scaleLinear()
        .domain([0, 304])
        .range([2 * step + 2 * step / 6, 1 * step + step / 6]);

    let yScale3 = d3.scaleLinear()
        .domain([0, 304])
        .range([3 * step + 3 * step / 6, 2 * step + step / 6]);

    let yScale4 = d3.scaleLinear()
        .domain([0, 304])
        .range([4 * step + 4 * step / 6, 3 * step + step / 6]);

    let yScale5 = d3.scaleLinear()
        .domain([0, 304])
        .range([5 * step + 5 * step / 6, 4 * step + step / 6]);
    ...
    pattern_g.append("g")
        ...
        pattern_g.append("g")
        .attr("transform", "translate(" + 0 + "," + (0 + yScale1(0)) + ")")
        .call(d3.axisBottom(xScale).ticks(20))
        .append("text")
        .attr("font-family", "Georgia")
        .attr('font-size', 15)
    ...
    pattern_g.append("g")
        .attr("transform", "translate(" + xScale(1) + "," + (0) + ")")
        .call(d3.axisLeft(yScale1).ticks(2))
        .append("text")
        .attr("font-family", "Georgia")
        .attr('font-size', 15)
        .attr("dx", "7em")
