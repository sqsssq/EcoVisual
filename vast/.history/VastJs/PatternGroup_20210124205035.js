    ...
    d3.csv("data/box_calc_rank.csv").then((data) => {
        d3.csv('data/treatment.csv').then(treatData => {
            ...
            for (let i = 0; i < data.length; ++i) {
                ...
                // else 
                //     data[i]['typeA'] = '+';
                if (parseInt(data[i].lun) == 10) console.log(1);
                if (parseInt(data[i].lun) == 10 && (parseFloat(data[i]['129']) == 10 || parseFloat(data[i]['129']) == 0 || parseFloat(data[i]['129']) < 0)) {
                    tname[data[i].code] = 1;
                }
                for (let j = 1; j < 5; ++j) {
            ...
            for (let i = 0; i < data.length; ++i) {
                for (let j in data[i].type) {
                    ...
                    if (tname[data[i].code] && parseInt(data[i].lun) > 10)
