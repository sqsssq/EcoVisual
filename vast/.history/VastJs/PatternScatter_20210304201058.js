...
.attr('class', 'svgBorder')
    ...
    .attr("height", heightScatter / 2 - 35)
    .attr('transform', 'translate(0, 35)');
let svgAxis = d3.select("#patternScatter").append("svg")
.attr('class', 'svgBorder')
.attr("width", widthScatter)
.attr("height", heightScatter / 2 - 30)
    ...
    d3.json('data/20210226.json').then((scatter_data) => {
        d3.csv('data/jingshouyi.csv').then((rect_data) => {
            ...
            const brush = d3.brush()
                .extent([
                    [5, 5],
                    [xAxisWidth - 5, yAxisWidth / 2 - 35 - 10]
                ])
                .on('brush', brushing)
                .on('end', brushed);
...
function DrawAxis() {
    d3.csv('data/box_calc_rank.csv').then((box_data) => {
        ...
        // console.log(y_array);
        let y_width = yAxisWidth / 2 - 30 - 15;
        let y_start = yAxisWidth / 2 - 30 - 10;
        ...
        for (let i in x_scale_array) {
            axis_g.append("g")
                .attr("transform", "translate(" + 0 + "," + (0 + y_scale_array[y_axis_type](0)) + ")")
                .call(d3.axisBottom(x_scale_array[i]).ticks(0))
            axis_g.append("g")
                .attr("transform", "translate(" + x_scale_array[x_axis_type](0) + "," + (0) + ")")
                .call(d3.axisLeft(y_scale_array[i]).ticks(0))
        }
    })
}
