var width = document.getElementById("DecisionGlyph").offsetWidth,
    height = document.getElementById("DecisionGlyph").offsetHeight;

var svg;

svg = d3.select("#DecisionGlyph").append("svg")
    .attr("width", width)
    .attr("height", height)
...
function paint() {
    d3.csv("data/box_calc_rank.csv").then((data) => {
        ...
        let decisionDiagonal = svg.append("g")
        ...
        // 绘制节点
        let decisionPoint = svg.append("g")
            .selectAll("#decision")
            .attr("id", "decision")
            .data(pointPaint)
            .enter()
