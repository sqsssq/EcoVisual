...
svgList = d3.select("#chart").append("svg")
    .attr("width", width)
    // .attr("height", height).attr('transform', 'translate(' + 0 + ',' + 50 + ')');
                ...
                d3.csv("data/treatment.csv").then((treatment) => {
                    ...
                    for (let i = 0; i < data.length; ++i) {
                        if (isNaN(parseInt(name[data[i].code])))
                            continue;
                        let arc = d3.arc()
                            .innerRadius(9 + wScale(parseFloat(data[i]['129'])))
                            .outerRadius(12 + wScale(parseFloat(data[i]['129'])));
                        ...
                        listG
                            ...
                            .enter()
                            .append('g')
                            .attr('transform', 'translate(' + (60 + parseInt(data[i].biao) * 55) + ',' + (50 * (name[data[i].code]) - 3) + ')')
                            .append('path')
                            .attr('d', d => {
                                // console.log(d);
                                return arc(d);
